...
public class Connection implements Runnable {
    ...
    public void changeSPeer(InetAddress oldPeer, InetAddress newPeer) throws IOException{
        InetAddress server = server = InetAddress.getByName("CMPLAB1-04");
        socket = new Socket(server, 55552);
        out = new ObjectOutputStream(socket.getOutputStream());
        in = new ObjectInputStream(socket.getInputStream());
        DisconnectMessage dcMessage = new DisconnectMessage("Disconnected.", "SP", oldPeer);
        send(dcMessage);
        SuperpeerMessage spMessage = new SuperpeerMessage("Become Superpeer", "SP", newPeer);
        send(spMessage);
        close();
    }
        ...
        try {
            ...
            while (active) {
                ...
                if(selection == 0){
                    ...
                    demo.startSPeer();
                    send(new SuperpeerMessage("Become Superpeer", "SP", socket.getLocalAddress()));
                }
                else{
